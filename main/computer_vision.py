...
"""
# camera settings
from resources import camera_matrix
from resources import dist_coefficients
...
def create_argparser():
    ...
    parser.add_argument('-v', '--visual', required=False, default=visual_debug, help='visual debug mode')
...
visual_debug = arg["visual"]

# setup UPD multicast
output_id = 1001
output_type = opendlv_sim_Frame()
...
def detect_cube_and_hand(img: np.ndarray, pat_cube: [int], pat_hand: [int], vis: bool,
    ...
    corners, ids, rejected_img_points = arc.detectMarkers(image=gray, dictionary=aruco, parameters=param)
