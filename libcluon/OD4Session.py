...
from libcluon.cluonDataStructures_pb2 import *
...
class OD4Session:
    ...
    def __init__(self, cid, port=12175, message_output=False):
        assert cid <= 255
    ...
    def __process(self, e):
        if self.message_output: print ("Received Envelope with ID = %s/%s" % ((e.dataType), str(e.senderStamp)))
